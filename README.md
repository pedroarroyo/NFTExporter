...
In order for NFTExporter to read your .blend file, you need to structure your scene in a specific way. Please follow all naming and collection conventions exactly, otherwise the scripts will not run properly. 

**Important Note**

Your .blend file must be moved to the NFTExporter folder. When you run the script, the .blend file must be in the directory of the NFTExporter folder. The Blender text editor has some weird quirks that make finding the right directory a bit tricky. If you are interested, I suggest reading about it in the Blender API above. This is the only work around I could find for now. 
...
You will need to change variables in the config.py file with a text editor or IDE; I recomend Visual Studio Code, but Blender has a bilt in Text Editor for ease of use. config.py is where you can customize aspects of your NFT collection and how it is generated.

Description of customisable variables to generate images: 

``nftName`` - A string representing name of the file exported


``imageFileFormat`` - A string representing the image file format that will export generated images as: (REQUIRED)
...
``animationFileFormat`` - A string representing the animations file format that ill export generated animations as: (REQUIRED)
...
``modelFileFormat`` - A string representing the 3D Models file format that will export generated 3D Modles as: (REQUIRED)
...
``save_path_mac`` - A string representing the save path if you are on MacOS: (REQUIRED - if on MacOS)

    Example mac: /Users/Path/to/NFTExporter


``save_path_windows`` - A string representing the save path if you are on Windows: (REQUIRED - if on Windows)

    Example windows: C:\Users\Path\to\NFTExporter
...
3. With the Blender File View open, navigate to the NFTExporter folder, navigate to the ``src`` folder, then open main.py.
...
The following section covers generating and exporting 3D models with NFTExporter with a repository of external 3D models in a folder structure. The 3D model generator combines 3D models together and exports all possible combinations of those 3D models to a folder. 
...
2. After you have formated the repository of 3D models to the above convention, copy and past it into the 3D_Model_Input folder located in NFTExporter: 
...
5. Save the .blend file to the NFTExporter folder and reload the .blend file from the new directory.
...
If you have 20 variants with 50 set as the rarity percentage for each, NFTExporter will add up the percentages then treat the sum as 100%:
